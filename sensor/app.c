...
RUI_GPIO_ST rui_gpio_4_vcc; //RAK811 pin to power humidity sensor
RUI_GPIO_ST rui_gpio_22_hum; //RAK811 pin to get humidity measure
uint8_t VccState = 0; //Enable/disable humidity power  
...
void app_loop(void)
{
    ...
    if(app_lora_status.IsJoined)  //if LoRaWAN is joined
    {
        if(autosend_flag)
        {
            ...
            VccState = 1;
			rui_gpio_rw(RUI_IF_WRITE, &rui_gpio_4_vcc, &VccState);
			rui_delay_ms(100); 
...
void main(void)
{
     ...
     * 
     * *****************************************************************************************/
    rui_gpio_4_vcc.pin_num = 4;
    rui_gpio_4_vcc.dir = RUI_GPIO_PIN_DIR_OUTPUT;
    rui_gpio_4_vcc.pull = RUI_GPIO_PIN_PULLDOWN; //Ground when idle
    rui_gpio_init(&rui_gpio_4_vcc);
